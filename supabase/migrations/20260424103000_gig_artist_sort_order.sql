...
alter table public.gig_artists
...
with ordered_artists as (
  select
    ...
    ga.artist_id,
    row_number() over (
      partition by ga.gig_id
      order by a.name, ga.created_at, ga.artist_id
    ) - 1 as sort_order
  from public.gig_artists ga
  join public.artists a on a.id = ga.artist_id
)
update public.gig_artists ga
set sort_order = ordered_artists.sort_order
from ordered_artists
where ordered_artists.gig_id = ga.gig_id
  and ordered_artists.artist_id = ga.artist_id
  and ga.sort_order is null;
