...
export class SupabaseGigStore implements GigStore {
  ...
  private readonly imageRepository = new SupabaseImageRepository(this.client);
  private readonly operationsRepository = new SupabaseOperationsRepository(
    this.client,
    this.venueCache,
    this.sourcePriorityCache
  );
  private readonly artistRepository = new SupabaseArtistRepository(
    this.client,
    (input) => this.ensureSource(input),
    (sourceIds) => this.buildSourcePriorityById(sourceIds)
  );

  async ensureImageBucket(): Promise<void> {
    await this.imageRepository.ensureBucket();
  }

  async listExpiredMirroredImageReferences(
    cutoffIso: string
  ): Promise<ImageCleanupObject[]> {
    return this.imageRepository.listExpiredReferences(cutoffIso);
  }

  async listOrphanedMirroredImageObjects(): Promise<ImageCleanupObject[]> {
    return this.imageRepository.listOrphanedObjects();
  }

  async deleteMirroredImageObjects(
    paths: string[]
  ): Promise<ImageCleanupDeleteResult> {
    return this.imageRepository.deleteObjects(paths);
  }

  async clearExpiredMirroredImageReferences(paths: string[]): Promise<number> {
    return this.imageRepository.clearExpiredReferences(paths);
  }
  ...
  async ensureSource(input: {
    ...
    return this.operationsRepository.ensureSource(input);
  }

  async startScrapeRun(sourceId: string, startedAt: string): Promise<string> {
    return this.operationsRepository.startScrapeRun(sourceId, startedAt);
  }
  ...
  async finishScrapeRun(
    ...
    await this.operationsRepository.finishScrapeRun(runId, result);
  }
  ...
  async syncGigArtistsFromSourceGigs(gigIds: string[]): Promise<void> {
    await this.artistRepository.syncFromSourceGigs(gigIds);
  }
  ...
  > {
    return this.artistRepository.repairActiveUpcoming(sources);
  }
}
